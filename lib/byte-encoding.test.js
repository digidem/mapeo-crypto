...
t.test('can encode and decode messages correctly', (t) => {
...
t.test('can encode and decode secret message with encryption key', (t) => {
  const data = {
    projectKey: crypto.randomBytes(32),
...
t.test('trying to decode an invalid buffer throws', (t) => {
...
t.test('trying to decode a different version throws', (t) => {
...
t.test('trying to decode a different type throws', (t) => {
  for (const [structName, data] of Object.entries(fixtures)) {
    const encoded = ByteEncoding[structName].encode(data)
