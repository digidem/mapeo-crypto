...
/**
 * Derive a 32-byte master key from the 18-byte identity key. We compromise
 * entropy (16 bytes vs 32) for the sake of an identity key that can be easily
 ...
 * hash the identity key, in the same way that derive-key hashes values, but by
 * using pwhash we increase security since it's more work to brute force how the
 * 16-bytes of entropy of the identity key map to the 32-bytes of entropy of the
 * master key.
 *
 * @param {Buffer} identityKey 16-bytes identity key
 * @returns {Buffer} 32-byte master key
 */
function deriveMasterKeyFromIdentityKey(identityKey) {
  assert(identityKey.length === 16, 'identityKey must be 16 bytes')
