 ...
 * @property {Buffer} identityKey 16-byte buffer - identity master key for device
 * @property {number} crc16 16-bit crc16 checksum of identityKey
 */
...
module.exports = {
  joinRequest: {
    /** @type {(joinRequest: JoinRequest) => Buffer} */
    encode(joinRequest) {
      return encode('joinRequest', joinRequest)
    },
    /** @type {(buf: Buffer) => JoinRequest} */
    decode(buf) {
  ...
  backupCode: {
    ...
    encode({ identityKey, crc16 }) {
      return Buffer.concat([identityKey, cenc.encode(cenc.uint16, crc16)])
    },
    /** @type {(buf: Buffer) => BackupCode} */
    decode(buf) {
      assert(buf.length === 16 + 2, 'Invalid backup code')
      const identityKey = buf.slice(0, 16)
      const crc16 = cenc.decode(cenc.uint16, buf.slice(16))
      return { identityKey, crc16 }
    },
  },
}
