...
import { ipv4Address } from 'compact-encoding-net'
...
function encode(structName, data) {
  return Buffer.from(cenc.encode(STRUCTS[structName], data))
}
...
function decode(structName, buf) {
  ...
  return /** @type {any} */ (
    Object.fromEntries(Object.entries(decoded).filter(([_, v]) => v !== null))
  )
}
...
export const inviteSecretMessage = {
